...
public function connection(){
	$conn_string = "host=localhost port=5432 dbname=projetPHP user=postgres password=ladent";
...
public function ajouterOffre($login, $titre,$type,$description,$dept,$nbpoints,$date){
	
	$result = pg_query($this->dbconn, "INSERT Into Formulaire(createur,titre,type,description,departement,point,date) VALUES ('$login','$titre','$type','$description','$dept','$nbpoints','$date');");
	$resultid = pg_query($this->dbconn, "select MAX(id) from Formulaire;");
	...
	foreach($_POST['domaine'] as $valeur){
		$result = pg_query($this->dbconn, "INSERT Into ListeFormulaireDomaine VALUES ($tabResult,'$valeur');");
				}
