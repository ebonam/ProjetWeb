    ...
    <div class="container">
      ...
      <!-- The justified navigation menu is meant for single line per list item.
		...
		<div class="input-group"> <span class="input-group-addon" id="basic-addon1">Titre de l'offre</span>	
			<input type="text" name="titre_offre" class="form-control" placeholder="Exemple : refaire une toiture" aria-describedby="basic-addon1">
		</div>
		...
		<div class="form-group">
			...
			<textarea class="form-control" rows="5" id="description" name="description_offre"></textarea>
		</div>
    </div>
 
	<!-- Entrées du prix -->
		<div class="form-group">
			<label for="usr">Prix (en points)</label>
			<input type="text" class="form-control" id="prixPoints"  name="prixOffre">
		</div>
		
	
		<!-- Entrées Departement -->	
		<div class="form-group">
        <label class="col-xs-3 control-label">Departement</label>
        <div class="col-xs-5 selectContainer">
            <select class="form-control" name="num_departement">
		...
		<br>
		 ...
		 <?php 
			$bdd = pg_connect("host=localhost port=5432 dbname=projetPHP user=postgres password=ladent");
			...
			echo <<<EOF
				<div class="row">
					<div class="col-sm-4 col-md-4">
							<div class="radio">
								<label><input type="checkbox" name="domaine[]" value={$row[0]} >&nbsp;{$row[0]}</label>
							</div>
					</div>
EOF;
